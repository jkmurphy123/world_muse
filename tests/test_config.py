...
def test_load_settings_creates_defaults_when_missing(tmp_path):
    ...
    assert settings.current_style == "Dramatic"
    assert settings.styles == ["Dramatic", "Relaxed", "Urgent", "Suspenseful", "Comedic", "Lurid", "Horror"]
...
def test_save_settings_persists_selected_values(tmp_path):
    ...
    assert loaded.current_style == "Suspenseful"
